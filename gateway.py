...
function uploadWhole(item,base,overwrite){return new Promise((resolve,reject)=>{const xhr=new XMLHttpRequest();xhr.open("POST", `/__upload__?path=${encodeURIComponent(join(base,item.path))}&overwrite=${overwrite?"1":"0"}`);xhr.withCredentials=true;xhr.upload.onprogress=e=>{if(!e.lengthComputable)return;item.current=e.loaded;item.pct=Math.min(100,Math.round(e.loaded/e.total*100));item.state="uploading";item.msg="Enviando";draw()};xhr.onload=()=>{if(xhr.status>=200&&xhr.status<300){resolve();return}let msg="Falha no upload";try{msg=(JSON.parse(xhr.responseText).error)||msg}catch(_){}reject(new Error(msg))};xhr.onerror=()=>reject(new Error("Conexao interrompida"));xhr.send(item.file)})}
function uploadChunk(item,base,overwrite,uploadId,offset,blob,part){return new Promise((resolve,reject)=>{const end=offset+blob.size;const xhr=new XMLHttpRequest();xhr.open("POST", `/__upload_chunk__?path=${encodeURIComponent(join(base,item.path))}&overwrite=${overwrite?"1":"0"}&upload_id=${encodeURIComponent(uploadId)}&offset=${offset}&total_size=${item.file.size}&complete=${end>=item.file.size?"1":"0"}`);xhr.withCredentials=true;xhr.upload.onprogress=e=>{if(!e.lengthComputable)return;item.current=offset+e.loaded;item.pct=Math.min(100,Math.round(item.current/item.file.size*100));item.state="uploading";item.msg=item.parts>1?`Parte ${part}/${item.parts}`:"Enviando";draw()};xhr.onload=()=>{if(xhr.status===201||xhr.status===202){try{const data=JSON.parse(xhr.responseText||"{}");if(typeof data.received==="number"){item.current=data.received;item.pct=Math.min(100,Math.round(item.current/item.file.size*100))}}catch(_){}resolve();return}let msg="Falha no upload";try{msg=(JSON.parse(xhr.responseText).error)||msg}catch(_){}reject(new Error(msg))};xhr.onerror=()=>reject(new Error("Conexao interrompida"));xhr.send(blob)})}
async function uploadChunkWithRetry(item,base,overwrite,uploadId,offset,blob,part){let lastError;for(let attempt=1;attempt<=3;attempt++){try{return await uploadChunk(item,base,overwrite,uploadId,offset,blob,part)}catch(err){lastError=err;if(attempt===3)break;item.msg=`Reconectando ${attempt}/3`;draw();await new Promise(resolve=>setTimeout(resolve,attempt*1500))}}throw lastError}
async function uploadOne(item,base,overwrite){if(item.file.size===0){await uploadWhole(item,base,overwrite);item.current=0;item.pct=100;item.state="done";item.msg="Concluido";s.done+=item.file.size;draw();return}const uploadId=makeUploadId();let offset=0;let part=0;while(offset<item.file.size){part+=1;const blob=item.file.slice(offset, Math.min(offset+CHUNK_BYTES,item.file.size));await uploadChunkWithRetry(item,base,overwrite,uploadId,offset,blob,part);offset+=blob.size;item.current=offset;item.pct=Math.min(100,Math.round(item.current/item.file.size*100));item.state="uploading";item.msg=offset<item.file.size?`Parte ${part}/${item.parts}`:"Finalizando";draw()}item.current=item.file.size;item.pct=100;item.state="done";item.msg="Concluido";s.done+=item.file.size;draw()}
